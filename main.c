...
//	Mag.Fuzzy_Out_filter = kalman_filter_2(Mag.Fuzzy_Out,500,10);
...
//	if(Mag.Fuzzy_Out_filter >= 0)
//	{
//		PID_UpdateSetVel(&M1, (1 - fabs(Mag.Fuzzy_Out_filter)) * Veh.Auto_Velocity);
//		PID_UpdateSetVel(&M2, (1 + fabs(Mag.Fuzzy_Out_filter)) * Veh.Auto_Velocity);
//	}
//	else
//	{
//		PID_UpdateSetVel(&M1, (1 + fabs(Mag.Fuzzy_Out_filter)) * Veh.Auto_Velocity);
		...
		if(VehStt.Veh_SampleState)
		{
			switch((int)Veh.Mode)
			{
				...
				case Manual_Mode: 
					...
					Mag.Fuzzy_Out = Defuzzification_Max_Min(Mag.Fuzzy_Error, Mag.Fuzzy_Error_dot);
//					Mag.Fuzzy_Out_filter = kalman_filter_1(Mag.Fuzzy_Out,500,10);
//					Mag.Fuzzy_Error = Mag.Set_Angle - Mag.Angle;
//					if(Mag.Fuzzy_Error > 180) 
//						Mag.Fuzzy_Error -= 360;
//					else if(Mag.Fuzzy_Error < -180) 
//						Mag.Fuzzy_Error += 360;					
//					Mag.Fuzzy_Out = Mag.Fuzzy_Error/160;
...
//					if(Mag.Fuzzy_Out_filter >= 0) // turn right
//					{
//						PID_UpdateSetVel(&M1, (1 - fabs(Mag.Fuzzy_Out_filter)) * Veh.Manual_Velocity);
//						PID_UpdateSetVel(&M2, (1 + fabs(Mag.Fuzzy_Out_filter)) * Veh.Manual_Velocity);
//					}
//					else // turn left
//					{
//						PID_UpdateSetVel(&M1, (1 + fabs(Mag.Fuzzy_Out_filter)) * Veh.Manual_Velocity);
