...
void GPS_StanleyControl(GPS *pgps, double v1_rpm, double v2_rpm)
{
	...
	pgps->heading_angle = Mag.Angle*(double)pi/180 + pi/2;
...
void	IMU_UpdateFuzzyInput(IMU *pimu)
{
	for(int i=0;i<4;i++)
	{
		...
		pimu->Fuzzy_Error_dot_pre+=pimu->Fuzzy_Error_dot;
	}
	pimu->Fuzzy_Error_dot = pimu->Fuzzy_Error_dot_pre/4;
	pimu->Fuzzy_Error_dot_pre =0;
}
...
enum_Error IMU_GetValueFromMessage(IMU *pimu, uint8_t *inputmessage)
{
	int temp = 100000;
	double Angle = 0;
	if(inputmessage[0] == 0x0D && inputmessage[IMU_AngleIndex - 1] == 0x20)
	{
		for(int i = 0; i < 6; i++)
		{
		...
		pimu->Angle_filter = kalman_filter(pimu->Angle,500,10);
		return Veh_NoneError;
	}
	else
	{
...
float kalman_filter(float z, float measure_noise, float process_noise)
{
...
float kalman_filter_1(float z1, float measure_noise1, float process_noise1)
{
	const float R1 = measure_noise1*measure_noise1;
	const float Q1 = process_noise1*process_noise1;
	static float x_hat1,P1;
	float P_1,K1;
	
		P_1 = P1 + Q1;
		K1 = P_1/(P_1 + R1);
		x_hat1 = x_hat1 + K1*(z1 - x_hat1);
		P1 = (1 - K1)*P_1;
	
	return x_hat1;
}
float kalman_filter_2(float z2, float measure_noise2, float process_noise2)
{
	const float R2 = measure_noise2*measure_noise2;
	const float Q2 = process_noise2*process_noise2;
	static float x_hat2,P2;
	float P_2,K2;
	
		P_2 = P2 + Q2;
		K2 = P_2/(P_2 + R2);
		x_hat2 = x_hat2 + K2*(z2 - x_hat2);
		P2 = (1 - K2)*P_2;
	
	return x_hat2;
}
float kalman_filter_3(float z3, float measure_noise3, float process_noise3)
{
	const float R3 = measure_noise3*measure_noise3;
	const float Q3 = process_noise3*process_noise3;
	static float x_hat3,P3;
	float P_3,K3;
	
		P_3 = P3 + Q3;
		K3 = P_3/(P_3 + R3);
		x_hat3 = x_hat3 + K3*(z3 - x_hat3);
		P3 = (1 - K3)*P_3;
